...
class MMDETDropBlock(nn.Module):
    ...
    def __init__(self, drop_prob, block_size, warmup_iters=2000, **kwargs):
        super(MMDETDropBlock, self).__init__()
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            mask_shape = (N, C, H - self.block_size + 1,
                          W - self.block_size + 1)
            mask = torch.bernoulli(
                torch.full(mask_shape, gamma, device=x.device))
        ...
        x = x * mask.numel() / (eps + mask.sum().item())
        del mask
        return x
